...
import casova_diskretizace_444 as casdis
...
ax.plot(x, E, label="$E$")
ax.plot(x, P_1S, label="$\sum P_1$")
ax.plot(x, P_2S, label="$\sum P_2$")
ax.plot(x, P_1S+P_2S, label="$\sum P_1 + \sum P_2$")
ax.plot(x, E-P_1S-P_2S, label="$E-\sum P_1 - \sum P_2$")
ax.legend(loc=2); # upper left corner
ax.set_xlabel('ČAS [s]',fontsize=10)
ax.set_ylabel('Akustická energie [J]',fontsize=10)

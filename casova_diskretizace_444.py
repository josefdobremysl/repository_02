import dirichletovaop_333 as dop
...
def CD(M0,K0,b0,mesh):
    ...
    ax.set_zlim3d(-np.ptp(zarray[:,pc]),np.ptp(zarray[:,pc])/6)#np.ptp(zarray[:,pc]))#np.ptp(zarray[:,pc])/1000 )
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('akustický tlak $ [10^{-3}Pa]$', rotation=180)
    plt.title("1")
    plt.axis('equal')
    plt.show()
    ...
    #ax.set_xlim3d(0,xlim)
    #ax.set_ylim3d(0, ylim)
    ax.set_zlim3d(-np.ptp(zarray[:,pc]),np.ptp(zarray[:,pc])/2)#np.ptp(zarray[:,pc]))#np.ptp(zarray[:,pc])/1000 )
    ax.plot_trisurf(mesh.nodeXY[:,0],mesh.nodeXY[:,1], zarray[:,pc], cmap=my_cmap, linewidth=0, edgecolor='none', antialiased=False)
    ax.set_box_aspect((asx,asy,asz))
    ax.view_init(elev=20, azim=-120, roll=0)
    ax.yaxis.set_ticks([0.,0.5,1.])
    ...
    #ax.set_xlim3d(0,xlim)
    #ax.set_ylim3d(0, ylim)
    ax.set_zlim3d(-np.ptp(zarray[:,pc]),np.ptp(zarray[:,pc])/2)#np.ptp(zarray[:,pc])/1000 )
    ax.plot_trisurf(mesh.nodeXY[:,0],mesh.nodeXY[:,1], zarray[:,pc], cmap=my_cmap, linewidth=0, edgecolor='none', antialiased=False)
    ax.set_box_aspect((asx,asy,asz))
    ax.view_init(elev=20, azim=-120, roll=0)
    ax.yaxis.set_ticks([0.,0.5,1.])
...
# ani.save(fn+'.gif',writer='imagemagick',fps=fps))
    ...
    file = open("obdelnik_Dirak_41.txt", "wb")
